 ...
 * value -- which is not zero. Sodium is milligrams; everything else is grams.
 *
 * The file is a verbatim copy of the dugcanlift-site repo's lift/foods.json,
 * built there by scripts/build_foods_json.py. That repo is the source of
 * truth: rebuild there and copy, never edit this copy. */
...
function parseFoodProduct(product) {
  ...
  const read = (suffix) => {
    ...
    return {
      ...
      // Null where the product does not say; sodium from salt when only salt
      // is listed (nutrients.js).
      ...CoachNutrients.fromOpenFoodFacts(nutriments, suffix),
    };
  };
